...
export const FamilyDashboard: React.FC = () => {
  ...
  const renderOverview = () => (
    <>
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{MOCK_PATIENT.age}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Condition:</span>
              <span className="font-medium">{MOCK_PATIENT.condition}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Room:</span>
              <span className="font-medium">{MOCK_PATIENT.room}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Admission Date:</span>
              <span className="font-medium">{format(MOCK_PATIENT.admissionDate, 'MMMM d, yyyy')}</span>
            </div>
          </div>
        </div>

        <div className="bg-white rounded-lg shadow p-6">
          <h3 className="text-lg font-semibold text-gray-700 mb-4">Assigned Caretaker</h3>
          <div className="flex items-center mb-4">
            <div className="w-12 h-12 bg-blue-100 rounded-full flex items-center justify-center text-blue-600 mr-4">
              <User size={24} />
            </div>
            <div>
              <h4 className="font-medium text-gray-900">{MOCK_CARETAKER.name}</h4>
              <p className="text-sm text-gray-600">{MOCK_CARETAKER.shift}</p>
            </div>
          </div>
          <div className="space-y-3">
            <div className="flex items-center">
              <Phone size={16} className="text-gray-500 mr-2" />
              <span className="text-gray-600">{MOCK_CARETAKER.phone}</span>
            </div>
            <div className="flex items-center">
              <MessageSquare size={16} className="text-gray-500 mr-2" />
              <span className="text-gray-600">{MOCK_CARETAKER.email}</span>
            </div>
            <div className="mt-4">
      ...
      <div className="mb-8">
        <h3 className="text-lg font-semibold text-gray-700 mb-4">Today's Care Summary</h3>
          ...
          <div className="bg-white rounded-lg shadow p-4">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-xl font-bold">{MOCK_TASKS.length}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
            ...
            {MOCK_UPDATES.map(update => (
              <div key={update.id} className="p-4">
                <div className="flex justify-between items-start mb-2">
                  <span className="font-medium">{update.caretaker}</span>
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600">Monitor care for {MOCK_PATIENT.name}</p>
      </div>
