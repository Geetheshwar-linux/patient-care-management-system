  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-600">Manage your assigned patients and tasks</p>
      </div>
      ...
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-gray-800 mb-4">My Patients</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {MOCK_ASSIGNED_PATIENTS.map(patient => (
            <div 
              key={patient.id} 
              className={`bg-white rounded-lg shadow p-4 border-l-4 cursor-pointer ${
                selectedPatient === patient.id ? 'border-blue-500' : 'border-gray-200'
              }`}
          ...
          <div 
            className={`bg-white rounded-lg shadow p-4 border-l-4 border-dashed flex items-center justify-center cursor-pointer ${
              selectedPatient === 'all' ? 'border-blue-500' : 'border-gray-200'
            }`}
            onClick={() => setSelectedPatient('all')}
          >
            <span className="text-gray-600">View All Patients</span>
          </div>
        </div>
      </div>
